	...
	function __construct()
	{
		...
		switch ($url) {
			...
			case '/paytm_response':
				 ...
				 $data = array("$keys[0]"=>$value[0],"$keys[1]"=>$value[1],"$keys[2]"=>$value[2],"$keys[3]"=>$value[3],"$keys[4]"=>$value[4],"$keys[5]"=>$value[5],"$keys[6]"=>$value[6],"$keys[7]"=>$value[7],"$keys[8]"=>$value[8],"$keys[9]"=>$value[9],"$keys[10]"=>$value[10],"$keys[11]"=>$value[11],"$keys[12]"=>$value[12],"$keys[13]"=>$value[13]);

				 $this->insert('paytm_payment',$data);
			...
			if(isset($_REQUEST['submit']))
				...
				include_once 'paytm_response.php';
				break;
			...
			case '/order-view-details':

				$where = array("status"=>'In Stock');
				$prod_arr = $this->select_join_where_multidata('product','category','product.cate_id=category.cate_id',$where);
				
				include_once 'order-view-details.php';
				break;
